-- Transfer selected Bell Art products to the Quorum brand and attach designer/collab metadata.
-- Safe to run more than once. No data is deleted.
...
DO $$
DECLARE
  quorum_brand_id uuid;
BEGIN
  SELECT id
    INTO quorum_brand_id
  FROM public.brands
  WHERE pg_temp.catalog_name_key(name) = 'quorum'
  ORDER BY created_at DESC
  LIMIT 1;

  IF quorum_brand_id IS NULL THEN
    INSERT INTO public.brands (name, segment)
    VALUES ('Quorum', 'high')
    RETURNING id INTO quorum_brand_id;
  END IF;
END $$;

WITH designer_names(designer_name) AS (
  VALUES
    ('Arthur Casas'),
    ('Crystian Freiberger'),
    ('Larissa Diegoli'),
...
INSERT INTO public.designers (name)
SELECT dn.designer_name
FROM designer_names dn
WHERE NOT EXISTS (
  SELECT 1
  FROM public.designers d
  WHERE pg_temp.catalog_name_key(d.name) = pg_temp.catalog_name_key(dn.designer_name)
);
...
matches AS (
  ...
  JOIN public.brands current_brand ON current_brand.id = p.brand_id
  JOIN product_map pm ON pg_temp.catalog_product_key(p.name) = pg_temp.catalog_name_key(pm.product_name)
  ...
  WHERE pg_temp.catalog_name_key(current_brand.name) IN ('bell art', 'bell arte')
     OR p.brand_id = tb.id
)
...
INSERT INTO public.product_style_tags (product_id, style_tag_id)
SELECT DISTINCT p.id, dst.id
FROM public.products p
JOIN product_map pm ON pg_temp.catalog_product_key(p.name) = pg_temp.catalog_name_key(pm.product_name)
JOIN target_brand tb ON tb.id = p.brand_id
JOIN public.design_style_tags dst
  ON pm.designer_name IS NOT NULL
 AND pg_temp.catalog_name_key(dst.name) = pg_temp.catalog_name_key(pm.designer_name)
ON CONFLICT (product_id, style_tag_id) DO NOTHING;
