...
export default function BrandCatalogPage() {
  ...
  return (
    <div className="min-h-screen flex">
      {brand && (
        <Helmet>
          <title>{`${brand.name} — Catálogo | Acervo 10.55`}</title>
          <meta name="description" content={`Coleção ${brand.name}: produtos, acabamentos e fichas técnicas para arquitetos e designers.`} />
          <link rel="canonical" href={`https://acervo1055.com.br/brand/${brand.id}`} />
          <meta property="og:title" content={`${brand.name} — Acervo 10.55`} />
          <meta property="og:url" content={`https://acervo1055.com.br/brand/${brand.id}`} />
        </Helmet>
      )}
      {/* Sidebar */}
