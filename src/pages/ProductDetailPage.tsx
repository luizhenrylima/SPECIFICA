...
function buildProductDescription(product: Product, brand: Brand | null) {
  ...
  return `${product.name}${brandText}${categoryText}. Produto selecionado para especificacao em projetos de arquitetura, com imagens, acabamentos e arquivos tecnicos reunidos no Acervo 10.55.`;
}
...
export default function ProductDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {product && (
        <Helmet>
          <title>{`${product.name}${brand ? ` - ${brand.name}` : ''} | Acervo 10.55`}</title>
          <meta name="description" content={productDescription.slice(0, 155)} />
          <link rel="canonical" href={`https://acervo1055.com.br/product/${product.id}`} />
          ...
          <meta property="og:url" content={`https://acervo1055.com.br/product/${product.id}`} />
